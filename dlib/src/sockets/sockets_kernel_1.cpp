...
// ----------------------------------------------------------------------------------------
    ...
    {
    public:
        SOCKET_container ( SOCKET s = INVALID_SOCKET) : val(s) {}

        SOCKET val;
        operator SOCKET&() { return val; }

        SOCKET_container& operator= ( const SOCKET& s ) { val = s; return *this; }

        bool operator== ( const SOCKET& s ) const { return s == val; }
    };
...
// -----------------
    ...
    )
    {
        ...
        try 
        {
			...
			std::lock_guard<std::mutex> M(sockets_kernel_1_mutex::startup_lock);
...
// ----------------------------------------------------------------------------------------
	/*
	connection::socket_descriptor_type connection::get_socket_descriptor () const
    {
