	...
	void *base, apr_size_t size)
{
	...
	if ((rv = apr_thread_mutex_lock(lock)) != APR_SUCCESS)
		return rv;

	(*rmm) = (apr_rmm_t *)apr_zalloc(sizeof(apr_rmm_t));
	(*rmm)->base = base;
	(*rmm)->size = size;
	(*rmm)->lock = lock;
...
APR_DECLARE(apr_status_t) apr_rmm_destroy(apr_rmm_t *rmm)
{
	...
	if ((rv = apr_thread_mutex_lock(rmm->lock)) != APR_SUCCESS) {
		return rv;
	}
	...
	return apr_thread_mutex_unlock(rmm->lock);
}
...
APR_DECLARE(apr_rmm_off_t) apr_rmm_malloc(apr_rmm_t *rmm, apr_size_t reqsize)
{
	...
	apr_thread_mutex_lock(rmm->lock);
	...
	apr_thread_mutex_unlock(rmm->lock);
	return this;
}
